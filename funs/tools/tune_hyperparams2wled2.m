function [maxResult, maxParams] = tune_hyperparams2wled2( ... 
...
%--------------------------------------------------------------------------
    ...
    fprintf(fid_csv, "t,num_pixels,p_value,gamma,lambda,projDim1,projDim2," + ...
                    ...
                    for lambda = lambdas
                        for proj1 = projDim{1}
                            for proj2 = projDim{2}
                                ...
                                remark      = '';
                                currProjDim = {proj1, proj2}; % 只有两个模态的投影维度
                                ...
                                fprintf(['dataset:%s, Loop:%d/%d, t:%g, num_pixels:%d, ' ...
                                         'p_value:%.2f, γ:%g, λ:%g, projDim:%d/%d, ' ...
                                         'ACC:%.4f\n'], ...
                                         dataType, currentLoop, totalLoops, ...
                                         t, num_pixel, p_value, gamma, lambda, ...
                                         proj1, proj2, results(1));

                                %%%% NEW/CHANGED ④：写 CSV 时包含 t
                                fprintf(fid_csv, ...
                                    '%g,%d,%.2f,%g,%g,%d,%d,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,"%s"\n', ...
                                    t, num_pixel, p_value, gamma, lambda, ...
                                    proj1, proj2, results(1), results(2), results(3), ...
